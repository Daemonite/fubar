...
(function($) {    
    ...
    $.fn.couchLogin = function(opts) {
        ...
        function initWidget() {
			$.couch.session({
				success: function(session){
					...
					if (userCtx.name) {
						...
						if (userCtx.roles.indexOf("_admin") != -1) {
							elem.html($.couchLogin.templates.adminParty);
						}
						else {
							elem.html($.couchLogin.templates.loggedOut);
							if (opts.loggedOut) {
								opts.loggedOut()
							}
						};
				...
				error: function(status, error, reason){
					if (opts.error) opts.error(reason);
				} 
			});
        };
        initWidget();
        ...
        elem.delegate("a[href=#signup]", "click", function() {
            elem.html($.couchLogin.templates.signupForm);
            elem.find('input[name="name"]').focus();
            return false;
        });
        elem.delegate("a[href=#login]", "click", function() {
            elem.html($.couchLogin.templates.loginForm);
            elem.find('input[name="name"]').focus();
            return false;
        });
